...
Phase-folding takes every observation's timestamp and replaces it with
where in the cycle (0 to 1) that observation falls, given a period.
If the period is right, scattered-looking data collapses into a clean
repeating shape -- this is the classic "does this period actually
explain the data" sanity check, and it's usually the next thing you'd
do right after compute_periodogram gives you a candidate period.
"""
...
def phase_fold(lc: LightCurve, period: float, t0: float = 0.0) -> LightCurve:
    ...
    t0 : float
        A reference epoch. Phase 0 corresponds to t0 (and every
        t0 + n*period). Defaults to 0.0, which is fine if you don't
        care about absolute phase alignment -- only relative shape.
